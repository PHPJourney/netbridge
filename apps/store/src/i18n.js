...
const dict = {
  zh: {
    ...
    "help.install.li3":
      "<strong>Windows Server</strong>（PowerShell <code>install.ps1</code> + WireGuard for Windows）",
    "help.install.body":
    ...
    "help.install.winBody":
      "在 <a href=\"#servers\">部署服务端</a> 的 Windows 卡片分别下载 <code>install.ps1</code> 与 <code>nbvpn-windows-amd64.exe</code>（2012 R2 用 win2012 产物），放进同一文件夹后执行安装命令。请先安装 <a href=\"https://www.wireguard.com/install/\" rel=\"noopener noreferrer\">WireGuard for Windows</a>。客户端出网 NAT 依赖 <code>New-NetNat</code> / RRAS，详见 <code>WINDOWS.md</code>；与 Linux iptables 完整对等仍可能需额外配置。",
    ...
    "dl.downloadInstallPs1": "下载 install.ps1",
    ...
    "dl.windowsSameFolder": "请将 install.ps1 与对应 exe 放在同一文件夹后再执行上方命令。",
  ...
  en: {
    ...
    "help.install.winBody":
      "On the Windows card under <a href=\"#servers\">Deploy server</a>, download both <code>install.ps1</code> and <code>nbvpn-windows-amd64.exe</code> (use the win2012 build on 2012 R2), place them in the same folder, then run the install command. Install <a href=\"https://www.wireguard.com/install/\" rel=\"noopener noreferrer\">WireGuard for Windows</a> first. Client egress NAT depends on <code>New-NetNat</code> / RRAS — see <code>WINDOWS.md</code>; full parity with Linux iptables may need extra setup.",
    ...
    "dl.downloadInstallPs1": "Download install.ps1",
    ...
    "dl.windowsSameFolder": "Put install.ps1 and the matching exe in the same folder, then run the command above.",
