...
const dict = {
  zh: {
    ...
    "clients.title": "下载客户端",
    "clients.lead": "Windows（仅 Win10+，不支持 Server 2012）、Android 可下载；macOS 源码本机运行（暂不分发）；iOS 暂无签名包。版本与 SHA256 来自发布清单。",
    "servers.title": "部署服务端",
    "servers.lead":
    ...
    "dl.downloadWinSetup": "下载服务端 Setup.exe（Win10+/2016+）",
    "dl.downloadWin2012Setup": "下载 Setup-win2012（2012：含 WG 0.5.3 + GUI）",
    "dl.downloadClientWinSetup": "下载客户端 Setup.exe（仅 Win10+）",
    "dl.downloadWinExe": "下载 nbvpn-windows-amd64.exe",
    "dl.downloadWin2012Exe": "下载 win2012 裸 exe（高级）",
    ...
    "dl.windowsClientHint": "仅 Windows 10 / 11。Server 2012 不能装客户端；部署节点请到下方「服务端 → Windows」用 Setup-win2012。",
    "dl.windowsSetupHint": "仅限 Windows 10 / Server 2016+。下载后右键「以管理员身份运行」。会自动安装 WireGuard（已装则跳过）。Server 2012 请用上方 Setup-win2012。",
    "dl.windows2012SetupHint": "Server 2012 / 2012 R2：双击本包安装。含 WireGuard 0.5.3 + Win32 GUI；开始菜单打开「NetBridge nbvpn GUI」。",
    "dl.windows2012Banner": "Server 2012 / 2012 R2：请下载 Setup-win2012 双击安装（含 WG 0.5.3 + GUI；不要用现代 Setup）。装好后开「NetBridge nbvpn GUI」。",
    "dl.windowsAdvanced": "高级 · Server 2012 / 无 GUI",
    "dl.windows2012Lead": "Server 2012 / 2012 R2：",
    "dl.windows2012Step1": "优先双击 NetBridge-nbvpn-Setup-win2012.exe",
    "dl.windows2012Step2Tail": "放到同一文件夹",
    "dl.windows2012Step2": "开始菜单 →「NetBridge nbvpn 管理」（数字菜单：状态/启停/peer/URI）",
    "dl.windows2012Step3": "无官方 WG 时 start 多为 dry-run；仍可导出 URI/conf。真隧道换 Linux / 2016+ 或自备旧 WG",
    "dl.windows2012Manage": "管理 = CLI / 管理菜单（无 Fyne）。常用：status · start · stop · peer list · peer add · show --uri",
    "dl.windows2012Cmd": "powershell -ExecutionPolicy Bypass -File .\\install.ps1 -SkipWireGuard",
    "dl.windowsBootstrapHint": "PowerShell 一键（管理员；自动识别 2012 与新系统）：",
    "dl.windowsSameFolder": "高级裸装：install.ps1 与 nbvpn-windows-amd64-win2012.exe 同目录；勿双击现代 Setup / GUI / 客户端安装包。",
    "dl.windowsSetupPrimary": "Win10+/2016+：Setup.exe。2012：Setup-win2012（管理菜单）或 exe + install.ps1。",
  ...
  en: {
    ...
    "clients.lead": "Windows (Win10+ only — not Server 2012) and Android downloads; macOS is local-source only (not distributed); iOS has no signed package yet. Versions and SHA256 come from the release manifest.",
    ...
    "dl.downloadWinSetup": "Download server Setup.exe (Win10+/2016+)",
    "dl.downloadWin2012Setup": "Download Setup-win2012 (2012: WG 0.5.3 + GUI)",
    "dl.downloadClientWinSetup": "Download client Setup.exe (Win10+ only)",
    "dl.downloadWinExe": "Download nbvpn-windows-amd64.exe",
    "dl.downloadWin2012Exe": "Download win2012 raw exe (advanced)",
    ...
    "dl.linkWinExe": "nbvpn-windows-amd64.exe",
    "dl.linkWindowsMd": "WINDOWS.md",
    "dl.windowsClientHint": "Windows 10 / 11 only. Server 2012 cannot run the client — deploy via Servers → Windows → Setup-win2012.",
    "dl.windowsSetupHint": "Windows 10 / Server 2016+ only. Right-click → Run as administrator. Auto-installs WireGuard when missing. On Server 2012 use Setup-win2012 above.",
    "dl.windows2012SetupHint": "Server 2012 / 2012 R2: double-click this package. Includes WireGuard 0.5.3 + Win32 GUI; Start Menu 「NetBridge nbvpn GUI」.",
    "dl.windows2012Banner": "Server 2012 / 2012 R2: download Setup-win2012 and double-click (WG 0.5.3 + Win32 GUI; not modern Setup). Then open 「NetBridge nbvpn GUI」.",
    "dl.windowsAdvanced": "Advanced · Server 2012 / headless",
    "dl.windows2012Lead": "Server 2012 / 2012 R2:",
    "dl.windows2012Step1": "Prefer double-click NetBridge-nbvpn-Setup-win2012.exe",
    "dl.windows2012Step2Tail": "into the same folder",
    "dl.windows2012Step2": "Start Menu →「NetBridge nbvpn 管理」(status / start-stop / peer / URI)",
    "dl.windows2012Step3": "Without official WG, start is usually dry-run; you can still export URI/conf. Real tunnels: Linux / 2016+ or legacy WG",
    "dl.windows2012Manage": "Manage via CLI / manage menu (no Fyne). Common: status · start · stop · peer list · peer add · show --uri",
    "dl.windows2012Cmd": "powershell -ExecutionPolicy Bypass -File .\\install.ps1 -SkipWireGuard",
    "dl.windowsBootstrapHint": "PowerShell one-liner (elevated; detects 2012 vs newer):",
    "dl.windowsSameFolder": "Advanced raw install: install.ps1 next to nbvpn-windows-amd64-win2012.exe — do not double-click modern Setup / GUI / client installer.",
    "dl.windowsSetupPrimary": "Win10+/2016+: Setup.exe. 2012: Setup-win2012 (manage menu) or exe + install.ps1.",
