...
const dict = {
  zh: {
    ...
    "dl.windowsClientHint": "仅 Windows 10 / 11。Server 2012 不能装客户端；部署节点请到下方「服务端 → Windows」高级区用 win2012 exe。",
    "dl.windowsSetupHint": "仅限 Windows 10 / Server 2016+。下载后右键「以管理员身份运行」。会自动安装 WireGuard（已装则跳过）。Server 2012 请勿下载本包。",
    "dl.windows2012Banner": "Server 2012 / 2012 R2：不要双击任何 Setup 安装。展开下方高级区：下载 win2012 exe + install.ps1，管理员 PowerShell 执行（可先跑 exe version 确认能启动）。",
    "dl.windowsAdvanced": "高级 · Server 2012 / 无 GUI（必看）",
    "dl.windows2012Lead": "Server 2012 / 2012 R2：勿用 Setup；管理员 PowerShell 安装（勿把 exe 当安装包双击完事）：",
    "dl.windows2012Step1": "下载 nbvpn-windows-amd64-win2012.exe",
    "dl.windows2012Step2Tail": "放到同一文件夹",
    "dl.windows2012Step3": "管理员 PowerShell：先 .\\nbvpn-windows-amd64-win2012.exe version，再执行下方命令",
    ...
    "dl.windowsSameFolder": "2012：install.ps1 与 nbvpn-windows-amd64-win2012.exe 同目录；勿双击 Setup / 现代 exe / GUI / 客户端安装包。",
    "dl.windowsSetupPrimary": "Win10+/2016+：Setup.exe。2012：仅 win2012 exe + 管理员 PowerShell 跑 install.ps1（-SkipWireGuard）。",
  ...
  en: {
    ...
    "dl.linkWinExe": "nbvpn-windows-amd64.exe",
    "dl.linkWindowsMd": "WINDOWS.md",
    "dl.windowsClientHint": "Windows 10 / 11 only. Server 2012 cannot run the client — deploy a node via Servers → Windows → Advanced (win2012 exe).",
    "dl.windowsSetupHint": "Windows 10 / Server 2016+ only. Right-click → Run as administrator. Auto-installs WireGuard when missing. Do not use on Server 2012.",
    "dl.windows2012Banner": "Server 2012 / 2012 R2: do not double-click any Setup. Expand Advanced below: download win2012 exe + install.ps1, run in elevated PowerShell (optional: run exe version first to confirm it starts).",
    "dl.windowsAdvanced": "Advanced · Server 2012 / headless (required)",
    "dl.windows2012Lead": "Server 2012 / 2012 R2: no Setup — install via elevated PowerShell (do not treat the exe as a double-click installer):",
    "dl.windows2012Step1": "Download nbvpn-windows-amd64-win2012.exe",
    "dl.windows2012Step2Tail": "into the same folder",
    "dl.windows2012Step3": "Elevated PowerShell: run .\\nbvpn-windows-amd64-win2012.exe version, then the command below",
    "dl.windows2012Cmd": "powershell -ExecutionPolicy Bypass -File .\\install.ps1 -SkipWireGuard",
    "dl.windowsBootstrapHint": "PowerShell one-liner (elevated; detects 2012 vs newer):",
    "dl.windowsSameFolder": "On 2012: install.ps1 next to nbvpn-windows-amd64-win2012.exe — do not double-click Setup / modern exe / GUI / client installer.",
    "dl.windowsSetupPrimary": "Win10+/2016+: Setup.exe. 2012: win2012 exe + elevated install.ps1 (-SkipWireGuard) only.",
