...
const dict = {
  zh: {
    ...
    "help.install.winBody":
      "在 <a href=\"#servers\">部署服务端</a> 的 Windows 卡片：<strong>Win10 / Server 2016+</strong> 下载 <strong>Setup.exe</strong> 并以管理员运行（自动装 WireGuard；可用 Fyne GUI）。<strong>Server 2012 / 2012 R2 不要用 Setup / 现代 exe / GUI</strong>（会英文报「不支持此 Windows 版本」）——展开「高级」下载 <code>nbvpn-windows-amd64-win2012.exe</code> + <code>install.ps1</code>，管理员执行 <code>install.ps1 -SkipWireGuard</code>（无 GUI；官方 WG 无真隧道，仅导出配置）。详见 <code>WINDOWS.md</code>。",
    ...
    "dl.windows2012Lead": "Server 2012 / 2012 R2：不要用上面的 Setup。请下载 win2012 exe + install.ps1，放同一文件夹后管理员执行：",
    ...
    "dl.windowsSameFolder": "2012：将 install.ps1 与 nbvpn-windows-amd64-win2012.exe 放同一文件夹；勿用 nbvpn-windows-amd64.exe / Setup / GUI。",
  ...
  en: {
    ...
    "help.install.winBody":
      "On the Windows card under <a href=\"#servers\">Deploy server</a>: <strong>Win10 / Server 2016+</strong> use <strong>Setup.exe</strong> as Administrator (bundles WireGuard; Fyne GUI). <strong>Server 2012 / 2012 R2: do not use Setup / modern exe / GUI</strong> (English “does not support this version of Windows”) — expand Advanced, download <code>nbvpn-windows-amd64-win2012.exe</code> + <code>install.ps1</code>, run <code>install.ps1 -SkipWireGuard</code> elevated (no GUI; official WG has no real tunnel). See <code>WINDOWS.md</code>.",
    ...
    "dl.windows2012Lead": "Server 2012 / 2012 R2: do not use Setup above. Download win2012 exe + install.ps1, same folder, elevated:",
    "dl.windows2012Cmd": "powershell -ExecutionPolicy Bypass -File .\\install.ps1 -SkipWireGuard",
    "dl.windowsBootstrapHint": "PowerShell one-liner (elevated; detects 2012 vs newer):",
    "dl.windowsSameFolder": "On 2012: put install.ps1 with nbvpn-windows-amd64-win2012.exe only — not modern exe / Setup / GUI.",
