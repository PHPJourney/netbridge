...
const dict = {
  zh: {
    ...
    "help.install.li3":
      "<strong>Windows Server</strong>（推荐 <code>NetBridge-nbvpn-Setup.exe</code>；高级可用 <code>install.ps1</code> + WireGuard）",
    "help.install.body":
      "到 <a href=\"#servers\">部署服务端</a> 复制对应系统的安装命令：Linux 在有公网 IP 的云主机上以 root / <code>sudo</code> 执行；Windows 在管理员 PowerShell 中执行。安装完成后会配置首个客户端 peer，并尽量启用服务。",
    "help.install.winTitle": "Windows 服务端 MVP",
    "help.install.winBody":
      "在 <a href=\"#servers\">部署服务端</a> 的 Windows 卡片优先下载 <strong>Setup.exe</strong>（<code>NetBridge-nbvpn-Setup.exe</code>）。高级/Server 2012 再下 <code>install.ps1</code> 与对应 exe。请安装 <a href=\"https://www.wireguard.com/install/\" rel=\"noopener noreferrer\">WireGuard for Windows</a> 后再开真隧道。数据目录在隐藏的 <code>%ProgramData%\\nbvpn</code>：可用 <code>explorer %ProgramData%\\nbvpn</code>。详见 <code>WINDOWS.md</code>。",
    ...
    "dl.windowsSameFolder": "若不用 Setup.exe：请将 install.ps1 与对应 exe 放在同一文件夹后再执行上方命令。",
  ...
  en: {
    ...
    "help.install.li3":
      "<strong>Windows Server</strong> (prefer <code>NetBridge-nbvpn-Setup.exe</code>; advanced: <code>install.ps1</code> + WireGuard)",
    "help.install.body":
      "Go to <a href=\"#servers\">Deploy server</a> and copy the install command for your OS: on Linux run as root / <code>sudo</code> on a host with a public IP; on Windows use an elevated PowerShell. Install configures the first client peer and tries to enable the service.",
    "help.install.winTitle": "Windows server MVP",
    "help.install.winBody":
      "On the Windows card under <a href=\"#servers\">Deploy server</a>, prefer <strong>Setup.exe</strong> (<code>NetBridge-nbvpn-Setup.exe</code>). For advanced / Server 2012 use <code>install.ps1</code> plus the matching exe. Install <a href=\"https://www.wireguard.com/install/\" rel=\"noopener noreferrer\">WireGuard for Windows</a> before a real tunnel. Data lives under hidden <code>%ProgramData%\\nbvpn</code> — open with <code>explorer %ProgramData%\\nbvpn</code>. See <code>WINDOWS.md</code>.",
    ...
    "dl.windowsSameFolder": "If not using Setup.exe: put install.ps1 and the matching exe in the same folder, then run the command above.",
