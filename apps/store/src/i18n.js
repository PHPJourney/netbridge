...
const dict = {
  zh: {
    ...
    "help.install.winBody":
      "在 <a href=\"#servers\">部署服务端</a> 的 Windows 卡片：大多数用户只下载 <strong>Setup.exe</strong>，右键「以管理员身份运行」。安装包会<strong>自动安装</strong>捆绑的 WireGuard for Windows（若尚未安装；已安装则跳过）。安装后可用开始菜单 <strong>NetBridge nbvpn GUI</strong>（启停/状态）或 CLI <code>nbvpn</code>。无 GUI / Server 2012 展开「高级」：2012 上官方 WG 不再支持真隧道（仅导出配置）。详见 <code>WINDOWS.md</code>。",
    ...
    "dl.windowsSetupHint": "下载后右键「以管理员身份运行」。安装包会自动安装 WireGuard 依赖（已装则跳过）。",
    ...
    "dl.windowsSetupPrimary": "推荐下载 Setup.exe 安装（含 WireGuard）；install.ps1 仅作高级/2012 备用。",
  ...
  en: {
    ...
    "help.install.winBody":
      "On the Windows card under <a href=\"#servers\">Deploy server</a>: most users only download <strong>Setup.exe</strong> and run as Administrator. The Setup <strong>auto-installs</strong> bundled WireGuard for Windows when missing (skips if already present). After install use Start Menu <strong>NetBridge nbvpn GUI</strong> (start/stop/status) or CLI <code>nbvpn</code>. Headless / Server 2012: expand Advanced — official WG no longer supports a real tunnel on 2012 (profiles only). See <code>WINDOWS.md</code>.",
    ...
    "dl.linkWinExe": "nbvpn-windows-amd64.exe",
    "dl.linkWindowsMd": "WINDOWS.md",
    "dl.windowsSetupHint": "After download, right-click → Run as administrator. Setup auto-installs WireGuard when missing (skips if present).",
    ...
    "dl.windowsSetupPrimary": "Prefer Setup.exe (includes WireGuard). install.ps1 is advanced / Server 2012 only.",
