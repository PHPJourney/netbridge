// EXAMPLE ONLY — copy into ios|macos/WGExtension after adding WireGuardKit SPM.
// Source: wireguard_flutter 0.1.3 example (MIT / WireGuard LLC notices retained below).
// Do not compile this path into the app until the Xcode target embeds WireGuardKit.
...
class PacketTunnelProvider: NEPacketTunnelProvider {
    ...
    override func startTunnel(options: [String : NSObject]?, completionHandler: @escaping (Error?) -> Void) {
        ...
        // bridge never loops through the utun (equivalent to the manual
        // `sudo route add -host <server> <gateway>`). The app embeds the
        // server IPs in the wgQuickConfig as a `# obfs-servers=` comment.
