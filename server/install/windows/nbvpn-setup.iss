...
[Messages]
WindowsVersionNotSupported=本安装包是服务端 Setup（NetBridge nbvpn），仅支持 Windows 10 / Server 2016 及以上，不能在 Server 2012 / 2012 R2 上运行。%n%n若您的系统是 Server 2012 / 2012 R2：请勿双击本 Setup 或客户端「网桥 VPN」Setup。请从 GitHub Releases 下载并双击：%n  • NetBridge-nbvpn-Setup-win2012.exe%n装好后用开始菜单「NetBridge nbvpn 管理」操作（无 Fyne GUI；无官方 WG 真隧道时多为 dry-run / 导出配置）。%n%n高级：也可下载 nbvpn-windows-amd64-win2012.exe + install.ps1。%n%n---%nThis is the *server* Setup (NetBridge nbvpn), Win10 / Server 2016+. On Server 2012 / 2012 R2 use NetBridge-nbvpn-Setup-win2012.exe from Releases, then Start Menu 「NetBridge nbvpn 管理」.
...
begin
  ...
  begin
    MsgBox(
      ...
      '  NetBridge-nbvpn-Setup-win2012.exe' + #13#10 +
      '装好后：开始菜单 →「NetBridge nbvpn 管理」' + #13#10#13#10 +
